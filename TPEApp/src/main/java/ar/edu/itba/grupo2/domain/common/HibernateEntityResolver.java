...
public class HibernateEntityResolver implements EntityResolver {
	...
	public Integer getId(final Object object) {
		Assert.isInstanceOf(EntityBaseType.class, object, "This entity resolver only hanldes objects implementing PersistentEntity");
		try {
			getSession().flush();
			Integer id = ((EntityBaseType) object).getId();
			if (id == null) {
